...
use burgerfilpper::strategies::avellaneda_stoikov_okex::AvellanedaStoikov;
...
async fn main() {
    ...
    let config: burgerfilpper::config::OkexConfig =
    ...
    let (tx, rx): (mpsc::Sender<WebsocketEvent>, mpsc::Receiver<WebsocketEvent>) = mpsc::channel();
