...
impl StrategyData {
    ...
    pub fn push(&mut self, event: BookTickerEvent) {
...
impl AvellanedaStoikov {
    pub fn new(config: Config) -> Self {
        ...
        let ema = Ema::new(config.ema_tick_period);
    ...
    pub async fn run_forever(&mut self, mut rx: mpsc::Receiver<FuturesWebsocketEvent>) {
        let account = self.account_client.account().await.unwrap();
        ...
        if qty > 0f64 {
            match self.account_client.market_sell(pair, qty).await {
                Ok(answer) => info!("market sell {:?}", answer),
                Err(err) => warn!("market sell Error: {}", err),
            }
        } else {
            match self.account_client.market_buy(pair, qty.abs()).await {
                Ok(answer) => info!("market sell {:?}", answer),
                Err(err) => warn!("market sell Error: {}", err),
            }
        }
    ...
    async fn on_tick(&mut self, data: BookTickerEvent) -> Result<()> {
        debug!("on_ticker: {:?}", data);
        self.strategy_data.push(data.clone());
        self.ema.run(*self.strategy_data.wap.back().unwrap());
    ...
    async fn on_account(&mut self, data: AccountUpdateEvent) -> Result<()> {
