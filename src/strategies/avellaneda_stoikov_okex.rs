...
impl StrategyData {
    ...
    pub fn push(&mut self, event: OrderBookEvent) {
...
impl AvellanedaStoikov {
    pub fn new(config: OkexConfig) -> Self {
            ...
            if let Some(event) = rx.recv().await {
                match event {
                    ...
                    WebsocketEvent::BalancePosition(balance_position_event) => {
                        ...
                        self.on_balance_position(balance_position_event).await;
                    }
                    WebsocketEvent::Order(order_event) => {
                        debug!("Order: {:?}", order_event);
                        self.on_order(order_event).await;
                    }
                    _ => {
    ...
    async fn on_position(&mut self, event: PositionsEvent) -> Result<()> {
    ...
    async fn on_order(&mut self, _event: OrderEvent) {}

    async fn on_balance_position(&mut self, _event: BalancePositionEvent) {}

    async fn on_tick(&mut self, _event: TickerEvent) {}

    async fn on_orderbook(&mut self, event: OrderBookEvent) -> Result<()> {
    ...
    async fn on_account(&mut self, event: AccountEvent) -> Result<()> {
